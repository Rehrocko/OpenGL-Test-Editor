...
// Make a list the size of however many keys there are in GLFW
bool    _keyPressed[GLFW_KEY_LAST + 1];
bool    _keyDown[GLFW_KEY_LAST + 1];
bool    _keyDownLastFrame[GLFW_KEY_LAST + 1];
double  _mouseX = 0;
double  _mouseY = 0;
...
void Input::Update() {
    ...
    // Keyboard
    for (int i = 32 /* Lowest GLFW_KEY enum */; i < GLFW_KEY_LAST + 1; i++) {
        // down
